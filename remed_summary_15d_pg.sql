-- ============================================================================
-- ชื่อไฟล์    : remed_summary_15d_pg.sql
-- ฐานข้อมูล  : HOSxP (PostgreSQL)
...
visit_dr AS (
  SELECT hn, vn, vstdate,
         string_agg(icode || ':' || qty, '|' ORDER BY icode) AS dr
...
visit_dx AS (
  SELECT vn, string_agg(DISTINCT icd10, ', ' ORDER BY icd10) AS dx
  FROM ovstdiag
  GROUP BY vn
),
last_pick AS (
  ...
  WHERE vd.vstdate BETWEEN CURRENT_DATE - INTERVAL '14 day' AND CURRENT_DATE
    AND NOT EXISTS (SELECT 1 FROM doctor_operation op WHERE op.vn = vd.vn)
    AND dx.dx ~ '(^|, )(I1[0-5]|E1[0-4])([0-9][0-9]?)?(,|$)'
),
prev_pick AS (
  ...
  JOIN visit_dr vd
   ...
   AND (lp.last_date - vd.vstdate) BETWEEN 20 AND 100
),
matched AS (
