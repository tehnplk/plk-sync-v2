-- คิวรี่สำหรับดึงข้อมูลสรุปการจ่ายยารายวันจากตาราง HOSxP (MySQL/MariaDB)
...
LEFT JOIN (
    SELECT 
        sub.vstdate,
        -- สร้างโครงสร้าง JSON Array เช่น [{"company": "Kerry", "count": 5}, ...]
        CONCAT(
            '[', 
            GROUP_CONCAT(
                CONCAT('{"company": "', IFNULL(sub.company_name, 'ไม่ระบุ'), '", "count": ', sub.qty, '}') 
                SEPARATOR ','
            ), 
            ']'
        ) AS company_json
